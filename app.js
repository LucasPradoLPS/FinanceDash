...
window.getUserPrefs = function(){ try{ const key=_prefsStorageKeyForSession(); const raw = localStorage.getItem(key); const p = raw? JSON.parse(raw) : {}; return { currency: p.currency || 'BRL', locale: p.locale || (p.currency==='BRL'?'pt-BR':'en-US'), theme: p.theme || 'light' }; } catch(e){ return { currency:'BRL', locale:'pt-BR', theme:'light' }; } };
